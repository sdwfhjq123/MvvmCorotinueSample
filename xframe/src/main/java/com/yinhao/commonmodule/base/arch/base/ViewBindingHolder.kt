package com.yinhao.commonmodule.base.arch.base
...
class ViewBindingHolder<B : ViewBinding> : ViewBindingHolderInterface<B>, LifecycleObserver {
    ...
    override fun bindViewBinding(binding: B, fragment: Fragment, onBound: (B.() -> Unit)?): View {
...
interface ViewBindingHolderInterface<B : ViewBinding> {
     ...
     * ### 绑定ViewBinding及其生命周期
     */
    fun bindViewBinding(binding: B, fragment: Fragment, onBound: (B.() -> Unit)?): View

    /**
     * ### 绑定ViewBinding及其生命周期
     */
    fun bindViewBinding(binding: B, activity: AppCompatActivity, onBound: (B.() -> Unit)?): View
